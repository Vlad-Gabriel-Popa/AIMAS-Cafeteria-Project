...
def main(yolo):
   ...
   # deep_sort 
    ...
    while True:
        ...
        t1 = time.time()
        frame_no += 1
        print("Reso", frame.shape[1], frame.shape[0])
        print("Frame:", frame_no, " Time:", time.time() - starttt, " fps:", frame_no / (time.time() - starttt))
       # image = Image.fromarray(frame)
        ...
        for track in list(tracker.tracks):
            ...
            if not track.exited and track.age > 60 and box[2] and box[3] and \
                    common_area((0, 0, frame.shape[1], frame.shape[0]), box) / (box[2]*box[3]) < 0.9:
                starting_box = track.start_to_tlwh()
                real_time = track.age/video_fps
                distance = abs(starting_box[0] - box[0])
                output_filee.write(str(distance) + " " + str(real_time))#+ " _" + str(starting_box[0]) + " " + str(box[0]) + "_ " + str(frame.shape[1] * real_time / distance)) #frame.shape[1] * real_time / distance
                output_filee.write("\n")
                track.exited = True
